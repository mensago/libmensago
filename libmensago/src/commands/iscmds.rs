...
pub fn addentry<V: VerifySignature>(
    ...
    let org_sig = match CryptoString::from(&resp.data["Organization-Signature"]) {
        Some(v) => v,
        None => {
            return Err(MensagoError::ErrServerException(format!(
    ...
    let prev_hash = match CryptoString::from(&resp.data["Previous-Hash"]) {
        ...
        None => {
            return Err(MensagoError::ErrServerException(format!(
                "Server exception: bad previous hash returned ({})",
    ...
    let hash = match CryptoString::from(&resp.data["Hash"]) {
        ...
        None => {
            return Err(MensagoError::ErrServerException(format!(
                "Server exception: bad entry hash returned ({})",
                &resp.data["Hash"]
            )))
        }
    };
    ...
    let req = ClientRequest::from(
        ...
        &vec![(
            "User-Signature",
            entry.get_authstr("User-Signature")?.as_str(),
        )],
    );
    conn.send(&req)?;
...
pub fn getwid(
    ...
    let mut req = ClientRequest::from("GETWID", &vec![("User-ID", uid.as_string())]);

    if domain.is_some() {
        req.data.insert(
            String::from("Domain"),
            String::from(domain.unwrap().as_string()),
        );
    }
    conn.send(&req)?;
...
pub fn iscurrent(
    ...
    let mut req = ClientRequest::from("ISCURRENT", &vec![("Index", index.to_string().as_str())]);

    if wid.is_some() {
        req.data.insert(
            String::from("Workspace-ID"),
            String::from(wid.unwrap().as_string()),
        );
    }
    conn.send(&req)?;
    ...
    match thread_rng().try_fill(&mut rawchallenge[..]) {
        ...
        Err(e) => {
            return Err(MensagoError::ErrProgramException(format!(
                "random number generator failure in login(): {}",
                e.to_string()
            )))
        }
    }
...
pub fn orgcard(conn: &mut ServerConnection, start_index: usize) -> Result<Keycard, MensagoError> {
    let req = ClientRequest::from(
        "ORGCARD",
        &vec![("Start-Index", start_index.to_string().as_str())],
    );
    ...
    if !resp.check_fields(&vec![("Total-Size", true), ("Item-Count", true)]) {
        return Err(MensagoError::ErrSchemaFailure);
    }
    ...
    out.entries
        .append(&mut parse_entries(&resp.data["Card-Data"])?);
...
pub fn password(conn: &mut ServerConnection, pwhash: &ArgonHash) -> Result<(), MensagoError> {
    let req = ClientRequest::from(
        "PASSWORD",
...
pub fn preregister(
    ...
    match wid {
        Some(w) => {
            req.data.insert(String::from("Workspace-ID"), w.to_string());
        }
        None => (),
    }
    ...
    match uid {
        Some(u) => {
            req.data.insert(String::from("User-ID"), u.to_string());
        }
        None => (),
    }
    ...
    match domain {
        Some(d) => {
            req.data.insert(String::from("Domain"), d.to_string());
        }
        None => (),
    }
    ...
    let outwid = match RandomID::from(resp.data["Workspace-ID"].as_str()) {
        Some(v) => v,
        None => {
            return Err(MensagoError::ErrServerException(String::from(
                "Server returned bad workspace ID",
            )))
        }
    };

    let outdom = match Domain::from(resp.data["Domain"].as_str()) {
        Some(v) => v,
        None => {
            return Err(MensagoError::ErrServerException(String::from(
                "Server returned bad domain",
            )))
        }
    };

    let outuid = match uid {
        Some(v) => Some(v.clone()),
        None => None,
    };
    ...
    Ok(PreregInfo {
        ...
        domain: outdom,
        uid: outuid,
        regcode: resp.data["Reg-Code"].clone(),
    })
}
...
pub fn regcode(
    ...
    if address.uid.get_type() == IDType::WorkspaceID {
        req.data
            .insert(String::from("Workspace-ID"), address.uid.to_string());
    } else {
        req.data
            .insert(String::from("User-ID"), address.uid.to_string());
    }
...
pub fn register(
        ...
        match resp.code {
            101 | 201 => {
                ...
                let domain = match resp.data.get("Domain") {
                    Some(s) => match Domain::from(s) {
                        Some(d) => d,
                        None => return Err(MensagoError::ErrBadValue),
                    },
                    None => return Err(MensagoError::ErrSchemaFailure),
                };
            ...
            408 => {
                // UID or WID exists
                if !resp.check_fields(&vec![("Field", true)]) {
                    return Err(MensagoError::ErrSchemaFailure);
                }

                match resp.data["Field"].as_str() {
                    "User-ID" => return Err(MensagoError::ErrExists),
                    "Workspace-ID" => {
    ...
    Err(MensagoError::ErrServerException(String::from(
        "Can't find a free workspace ID",
    )))
}
...
pub fn reset_password(
    ...
    let mut req = ClientRequest::from(
        "RESETPASSWORD",
        &vec![("Workspace-ID", wid.to_string().as_str())],
    );

    match reset_code {
        Some(r) => {
            req.data.insert(String::from("Reset-Code"), String::from(r));
        }
        None => (),
    }
    ...
    match expires {
        Some(x) => {
            req.data.insert(String::from("Expires"), x.to_string());
        }
        None => (),
    }
...
pub fn setstatus(
    ...
    match status {
        ...
        _ => return Err(MensagoError::ErrBadValue),
    }
