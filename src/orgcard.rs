...
mod tests {
	...
	fn orgentry_hash_sign_verify() -> Result<(), MensagoError> {
		...
		// Because orgentry_make_compliant_card() hashes and signs the entry it generates, we just
		...
		match orgentry.get_authstr(&AuthStrType::Organization) {
			Ok(v) => {
				if v.to_string() != "ED25519:^5K}!^1PCA*)Y$DZ9Slttai8t!P*40`xhq&I3-YN0@>*`!DH-hAr#@p?66DNP_RQ&pe4RdJ2#I{#*hpN" {
					return Err(MensagoError::ErrProgramException(
