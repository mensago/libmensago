...
mod tests {
    ...
    fn test_kcresolver() -> Result<(), MensagoError> {
        ...
        let admin_addr = match resolver
            .resolve_address(&MAddress::from("admin/example.com").unwrap(), &mut dh)
        {
            Ok(v) => v,
            Err(e) => {
                return Err(MensagoError::ErrProgramException(format!(
                    "{}: failed to resolve admin address: {}",
                    testname,
                    e.to_string()
                )))
            }
        ...
        if orgcard.entries.len() != 2 {
            return Err(MensagoError::ErrProgramException(format!(
                "{}: wrong org card entry count: wanted 2, got {}",
                testname,
                orgcard.entries.len()
            )));
        }
        ...
        if usercard.entries.len() != 1 {
            return Err(MensagoError::ErrProgramException(format!(
                "{}: wrong user card entry count: wanted 1, got {}",
                testname,
                orgcard.entries.len()
            )));
        }
    ...
    fn test_lookup_aaaa() -> Result<(), MensagoError> {
        ...
        let example_com = IpAddr::V6(Ipv6Addr::new(0x2620, 0x12e, 0x1000, 0, 0, 0, 0xa00, 0xf));
        let mut real_handler = DNSHandler::new_default().unwrap();
        let returned_ip =
            match real_handler.lookup_aaaa(&Domain::from("v6.testmyipv6.com").unwrap()) {
                Ok(v) => v,
                Err(e) => {
                    return Err(MensagoError::ErrProgramException(format!(
                        "{}: error looking up address: {}",
    ...
    fn test_lookup_srv() -> Result<(), MensagoError> {
        ...
        match real_handler.lookup_srv(&String::from("_mensago._tcp.mensago.net.")) {
            Ok(v) => v,
            Err(e) => {
                return Err(MensagoError::ErrProgramException(format!(
                    "{}: error getting SRV records for mensago.mensago.net: {}",
                    testname,
                    e.to_string()
                )))
            }
        ...
        let mut fake_handler = FakeDNSHandler::new();
        let records = match fake_handler.lookup_srv(&String::from("_mensago._tcp.example.com.")) {
            Ok(v) => v,
            Err(e) => {
                return Err(MensagoError::ErrProgramException(format!(
                    "{}: error getting fake SRV records for example.com: {}",
                    testname,
                    e.to_string()
                )))
            }
    ...
    fn test_lookup_txt() -> Result<(), MensagoError> {
        ...
        // mensago.net is currently used for testing and development purposes, so we can count
        // on this returning specific values
        let records = match real_handler.lookup_txt(&Domain::from("mensago.mensago.net").unwrap()) {
            Ok(v) => v,
            Err(e) => {
                return Err(MensagoError::ErrProgramException(format!(
                    "{}: error getting TXT records for mensago.net: {}",
                    testname,
                    e.to_string()
                )))
            }
        ...
        if records.len() != 2 {
            return Err(MensagoError::ErrProgramException(format!(
                "{}: record count mismatch: wanted 2, got {}",
                testname,
                records.len()
            )));
        }
        ...
        let mut fake_handler = FakeDNSHandler::new();
        let records = match fake_handler.lookup_txt(&Domain::from("mensago.mensago.net").unwrap()) {
            Ok(v) => v,
            Err(e) => {
                return Err(MensagoError::ErrProgramException(format!(
                    "{}: error getting fake TXT records for mensago.net: {}",
                    testname,
                    e.to_string()
                )))
            }
    ...
    fn test_lookup_mgmtrec() -> Result<(), MensagoError> {
        ...
        let rec = match get_mgmt_record(&Domain::from("example.com").unwrap(), &mut dh) {
            ...
            Err(e) => {
                return Err(MensagoError::ErrProgramException(format!(
                    "{}: error getting fake mgmt record for example.com: {}",
                    testname,
                    e.to_string()
                )))
            }
    ...
    fn test_get_server_config() -> Result<(), MensagoError> {
        ...
        let mut fakeconfig = match get_server_config(&Domain::from("example.com").unwrap(), &mut dh)
        {
            ...
            Err(e) => {
                return Err(MensagoError::ErrProgramException(format!(
                    "{}: error getting fake SRV-based record for example.com: {}",
                    testname,
                    e.to_string()
                )))
            }
        ...
        fakeconfig = match get_server_config(&Domain::from("example.com").unwrap(), &mut dh) {
            ...
            Err(e) => {
                return Err(MensagoError::ErrProgramException(format!(
                    "{}: error getting fake A-based record for example.com: {}",
                    testname,
                    e.to_string()
                )))
            }
